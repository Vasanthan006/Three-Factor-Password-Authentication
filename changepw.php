...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($new_password !== $confirm_password) {
        echo "<script>alert('Passwords do not match!'); window.location.href = 'changepw.php?user_id=$user_id';</script>";
        exit();
    }

    if (empty($img1) || empty($img2) || empty($img3)) {
        echo "<script>alert('Please select all three images!'); window.location.href = 'changepw.php?user_id=$user_id';</script>";
        exit();
    }
    ...
    if ($stmt->execute()) {
        ...
        echo "<script>alert('Error updating password. Please try again.'); window.location.href = 'changepw.php?user_id=$user_id';</script>";
    }
